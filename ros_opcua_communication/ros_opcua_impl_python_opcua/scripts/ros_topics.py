...
class OpcUaROSTopic:
    ...
    def _recursive_create_items(self, parent, idx, topic_name, type_name, message, top_level=False):
        ...
        # Detecta tipos complejos (estructuras)
        if hasattr(message, '__slots__') and hasattr(message, '_slot_types'):
            ...
            new_node.add_property(
                ua.NodeId(topic_name + ".Type", idx),
                ua.QualifiedName("Type", parent.nodeid.NamespaceIndex),
                type_name
            )
            if top_level:
                new_node.add_method(
                    ua.NodeId(topic_name + ".Update", parent.nodeid.NamespaceIndex),
                    ua.QualifiedName("Update", parent.nodeid.NamespaceIndex),
                    self.opcua_update_callback, [], []
                )
            for slot_name, type_name_child in zip(message.__slots__, message._slot_types):
            ...
            # Si el mensaje es un array de strings, crear nodos individuales para cada elemento
            for i, value in enumerate(message):
                element_name = "{}[{}]".format(topic_name, i)
                element_node = parent.add_variable(
                    ua.NodeId(element_name, parent.nodeid.NamespaceIndex),
                    ua.QualifiedName(element_name, parent.nodeid.NamespaceIndex),
                    value
                )
                element_node.set_writable(True)
                self._nodes[element_name] = element_node

        else:
            # Aqui se procesan tipos normales y arrays de otro tipo
            base_type_str, array_size = _extract_array_info(type_name)
            try:
                base_instance = roslib.message.get_message_class(base_type_str)()
            except (ValueError, TypeError):
    ...
    #     # Si el mensaje es un array, desglosalo en nodos individuales
    #     elif isinstance(message, (list, tuple)):
    #         for index, item in enumerate(message):
    #             array_element_name = f"{topic_name}_{index}"
    #             if array_element_name not in self._nodes:
    #                 # Crea un nodo para cada elemento del array si no existe
    #                 self._nodes[array_element_name] = parent.add_variable(
    #                     ua.NodeId(array_element_name, self.idx),
    #                     ua.QualifiedName(array_element_name, self.idx),
    #                     item
    #                 )
    #                 self._nodes[array_element_name].set_writable(True)  # Hacer que el nodo sea modificable
    #             else:
    #                 # Actualiza el valor del nodo si ya existe
    #                 self._nodes[array_element_name].set_value(item)
    #     else:
    #         if topic_name in self._nodes and self._nodes[topic_name] is not None:
    #             self._nodes[topic_name].set_value(repr(message))

    def update_value(self, topic_name, message):
        # Si el mensaje es una lista de strings, actualizar cada nodo del array
        if isinstance(message, list) and all(isinstance(item, str) for item in message):
            for i, value in enumerate(message):
                element_name = "{}[{}]".format(topic_name, i)
                if element_name in self._nodes:
                    self._nodes[element_name].set_value(value)

        elif hasattr(message, '__slots__') and hasattr(message, '_slot_types'):
            # Procesar estructuras complejas de datos
            for slot_name in message.__slots__:
                self.update_value(topic_name + '/' + slot_name, getattr(message, slot_name))

        elif type(message) in (list, tuple):
            # Procesar arrays de otro tipo (ej. float, int)
            if (len(message) > 0) and hasattr(message[0], '__slots__'):
                for index, slot in enumerate(message):
                    if topic_name + '[%d]' % index in self._nodes:
                        self.update_value(topic_name + '[%d]' % index, slot)
                        ...
                        if topic_name in self._nodes:
                            base_type_str, _ = _extract_array_info(self._nodes[topic_name].text(self.type_name))
                            self._recursive_create_items(self._nodes[topic_name], topic_name + '[%d]' % index, base_type_str,
                                                        slot, None)

            if topic_name in self._nodes and len(message) < len(self._nodes[topic_name].get_children()):
                for i in range(len(message), self._nodes[topic_name].childCount()):
                    item_topic_name = topic_name + '[%d]' % i
                    self.recursive_delete_items(self._nodes[item_topic_name])
                    del self._nodes[item_topic_name]
        else:
            # Actualizar nodos individuales que no son arrays ni estructuras complejas
            if topic_name in self._nodes and self._nodes[topic_name] is not None:
                self._nodes[topic_name].set_value(repr(message))
